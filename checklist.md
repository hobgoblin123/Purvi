...
- [ ] **nginx `add_header` inheritance** — Applies when adding or editing an nginx location block. Any `add_header` in a location replaces *all* server-level `add_header` directives for that location — security headers silently disappear. Explicitly duplicate `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy` (and `Content-Security-Policy` for finance) in every location block that uses `add_header`. Verify: `curl -sI <url> | grep -iE "x-frame|x-content|referrer|permissions|content-security"`.
- [ ] **CSP inline script audit** — Applies before adding a `Content-Security-Policy` header to any service. `script-src 'self'` blocks inline `<script>` tags — grep every HTML file the service serves: `grep -rn "<script" <service>/frontend/public/`. Extract any inline scripts to external `.js` files first, add a no-auth nginx location for each, then enable CSP.
